...
def run_retrain_acc(train_num, lr, lang, adaptor_save_dir, train_file_path, infer_file_path, output_infer_file, best_path, init_adapter_path,
                    re_num_epochs, re_batch_size, tokenizer_path, base_model_path):
    print("run")
    print("init_adapter_path: ", init_adapter_path)
    # 配置
    ...
    model.load_adapter(adapter_name_or_path=init_adapter_path, load_as="bottleneck_adapter", leave_out=best_path, with_head=True)
    ...
    for epoch in range(num_epochs):
        ...
        current_acc = validate_acc(tokenizer, model, valid_file_path, device)

        print("epoch: ",epoch)
        print('currnt acc %.8f, max acc %.8f' % (current_acc, max_acc))

        #保存最优
        if(current_acc > max_acc):
            model.save_adapter(adaptor_save_dir, "bottleneck_adapter") #如果目前的model好于max_mrr，那么就存为best。  如果后续没超过，则不更新best，否则更新
            max_acc = current_acc
            print('max acc %.8f' % (max_acc))
